...
// call sync or async child process
function __exec(file, args, sync, options, callback){
	if( sync ){
		var res = runSync( file + ' ' + args.join( ' ' ) );
		callback( null, res );
		return res;
	}else{
		exec2.call( null, file, args, options, callback );
	}
}
...
function parseIdentify(input) {
  ...
  for( var i=0, l=lines.length; i<l; i++ ){
    currentLine = lines[i];
    if( !currentLine.length || !~currentLine.indexOf(':') ){ continue; }

    indent = currentLine.search(/\S/);
    comps  = currentLine.split(':');

    if( comps.length > 1 ){ comps[1] = comps[1].trim(); }
    comps[0] = comps[0].trim().toLowerCase();

    if( !comps[1] ){ // line == "Property:", i.e. a parent property
      indents.push( prevIndent );
      prevIndent = indent;

      // push current prop onto the stack
      stack.push( prop );
      prop = prop[ comps[0] ] = {};
...
exports.identify = function(pathOrArgs, callback) {
  ...
  var sync = !callback
    ,  ret = undefined
    , proc = __exec(exports.identify.path, args, sync, {timeout:120000}, function(err, stdout, stderr) {
    var result, geometry;
    if (!err) {
      if (isCustom) {
        result = stdout;
...
exports.convert = function(args, timeout, callback) {
  ...
  return __exec(exports.convert.path, args, !callback, procopt, callback);
}
